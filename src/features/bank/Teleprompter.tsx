...
// Padding is a % of the scroller's width so one setting works on both a
// phone and a wide monitor. The slider's max is derived from the measured
// scroller width: padding may never squeeze the text column below
// MIN_TEXT_PX, so a monitor allows up to the 40% cap while a phone caps
// much lower. A fixed % cap alone was wrong — 40% per side leaves plenty
// of column on a monitor but ~80px on a phone.
const MIN_TEXT_PX = 240
const PADDING_PCT_CAP = 40
  ...
  try {
    ...
    if (raw) {
      ...
      if (stored.padding != null && stored.paddingPct == null) {
        ...
        stored.paddingPct = pct > PADDING_PCT_CAP ? DEFAULT_PREFS.paddingPct : pct
      }
...
export function Teleprompter({
  ...
  const [maxPaddingPct, setMaxPaddingPct] = useState(PADDING_PCT_CAP)
  ...
  // The padding slider's max comes from the scroller's own measured width
  // (the container the padding applies to): the largest % per side that
  // still leaves a MIN_TEXT_PX text column. Observed, not computed once,
  // because rotation and the md+ rail opening/closing resize the scroller.
  useLayoutEffect(() => {
    if (!open) return
    ...
    const measure = () => {
      ...
      if (!w) return
      const pct = Math.floor(((w - MIN_TEXT_PX) / 2 / w) * 100)
      setMaxPaddingPct(Math.max(0, Math.min(PADDING_PCT_CAP, pct)))
    }
    measure()
  ...
  // Apply the clamped value but keep the stored pref as-is, so a padding
  // chosen on a wide monitor survives a phone session instead of being
  // permanently narrowed the first time the phone clamps it.
  const paddingPct = Math.min(prefs.paddingPct, maxPaddingPct)
