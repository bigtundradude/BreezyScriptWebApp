...
function loadPrefs(): Prefs {
  try {
    ...
    if (raw) {
      const stored = JSON.parse(raw) as Partial<Prefs> & { width?: number }
...
export function Teleprompter({
  ...
  // paragraph's start, → advances to the next paragraph's start (both place
  // the target where it has just barely entered the screen, so reading
  // resumes naturally). Space/arrows are ignored while a control has focus
  // so they still operate that control.
  ...
  useEffect(() => {
    ...
    const seekParagraph = (dir: -1 | 1) => {
      ...
      for (let i = 0; i < paragraphs.length; i++) {
        if ((paragraphs[i] as HTMLElement).offsetTop > center) break
        index = i
      }
      if (dir === 1 && ++index >= paragraphs.length) return
      const target = paragraphs[index] as HTMLElement
      el.scrollTop = clampScroll(
        el,
        prefs.mirror
          ? el.scrollHeight - target.offsetTop - line
          : target.offsetTop - el.clientHeight + line,
      )
    }
    const onKey = (e: KeyboardEvent) => {
      ...
      if (target?.closest('input, button, textarea, select, [contenteditable]')) return
      e.preventDefault()
      if (e.key === ' ') setPlaying((p) => !p)
      else seekParagraph(e.key === 'ArrowLeft' ? -1 : 1)
    }
    ...
    <div className="fixed inset-0 z-300 flex flex-col bg-bg">
      ...
      <div className="flex shrink-0 items-center gap-2 border-b border-border-subtle px-3 py-1.5 md:px-4">
        ...
        <span className="hidden text-xs text-text-muted md:inline">
          Space play/pause · ←/→ paragraph · Esc close
        </span>
        <button
      ...
      <div className="relative flex min-h-0 flex-1">
        ...
        <div
          ...
          style={{ paddingLeft: prefs.padding, paddingRight: prefs.padding }}
        >
