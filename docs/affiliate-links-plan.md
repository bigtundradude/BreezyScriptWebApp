...
- **Link card** (in the list, no separate detail page needed to copy):
  ...
  - Then one row per URL: tag badge, truncated URL, and two copy actions, each a
    ...
    2. **Copy with title** — puts `{shortTitle} → {url}` on the clipboard (short
       title, space, right arrow U+2192, space, URL).
...
1. Copy-with-title separator is `→` (U+2192) with spaces. If the target platform
   mangles that character, a hyphen fallback is a one-line change.
