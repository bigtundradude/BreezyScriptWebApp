...
- **Link card** (in the list, no separate detail page needed to copy):
  ...
  - Then one row per URL: tag badge, truncated URL, and two copy actions, each a
    ...
    2. **Copy with title** — puts `{shortTitle} → {url}` on the clipboard: the short
       title, then the rightwards arrow U+2192 (`&rarr;`) with exactly one space on
       each side of it, then the URL.
...
1. Copy-with-title separator is `→` (U+2192, `&rarr;`, the rightwards arrow) with one
   space on both sides (owner, 2026-08-18, superseding the earlier U+27F6 pick). If a
   target platform mangles that character, a hyphen fallback is a one-line change.
