...
- **Compact list-page toolbar (decided 2026-08; use for new list screens):** skip the in-page page heading; content starts at a back link plus the toolbar. The toolbar is one row: a **debounced live search input** (250ms via `useDebouncedValue`, written to the URL with `replace: true`, clearing the box resets — no Search button, no "press Enter" placeholder; owner, 2026-08-18, superseding explicit-submit) + a filter-toggle icon button that shows/hides the filter selects on the row beneath (panel starts open when the URL carries an active filter; the toggle renders `primary` while any filter is active so hidden filters stay visible). The primary create action is a floating action button (round, fixed bottom-right, `md:hidden`) on phones and a labeled `primary` button at the right end of the toolbar on `md+` where the sidebar shows — never both at once; give the list a bottom spacer (`h-14 md:hidden`) so the FAB can't cover the last row. Reference implementations: `src/routes/c.$channelId.bank.index.tsx`, `src/routes/c.$channelId.brain.index.tsx`.
- **State-driven editor action bar (decided 2026-08; use for all editor views):** editors dock a sticky bottom bar (`sticky bottom-0`, `border-t`, page background) inside the scroll column. Its buttons reflect the document state, not a fixed layout. **Save NEVER navigates (owner, 2026-08-21): clicking Save (or Cmd/Ctrl+S) saves and stays on the page — everywhere.** The only buttons that navigate are `[Ready]` (workflow steps → next step), `[Close]`/`[Cancel]`, and the header back link. Saving a NEW entity swaps the editor to the created record (URL replace) without leaving the editing context. **Non-workflow editors** — dirty: `[Delete] … [Cancel] [Save]`, Cancel guarded by the unsaved-changes confirm; clean: `[Close]`. **Workflow steps**: a primary `[Ready]` is ALWAYS visible at the far right — enabled whenever the step's criteria pass (validated against live local state), disabled with the missing-criteria hint otherwise; clicking it saves any unsaved edits, marks the step ready if needed, and advances to the NEXT step (the last step returns to the idea overview). While dirty, `[Cancel] [Save]` appear beside Ready. Never render a disabled Save, and never label the exit "Cancel" when there is nothing to cancel. Delete sits left, separated from the exit/save group, and always goes through the confirm dialog. Reference implementation: `src/features/bank/BankIdeaEditor.tsx`.
...
- **Keep the native sync log current (owner, 2026-08-21):** every behavior-affecting change to the app must append an entry to `docs/native-sync-log.md` in the same commit — behavior described platform-agnostically, web file references, porting notes. That log is how Claude keeps the native version of this app in sync; a behavior change without a log entry is an incomplete change.
